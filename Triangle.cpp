...
void Triangle::TriangleType()const
{
	...
	if (AC == BC || AC == AB || AB == BC)std::cout << "Тръгълникът е равнобедрен." << std::endl;
	if (AC == BC&&AC == AB&&AB == BC)std::cout << "Триъгълникът е равностранен." << std::endl;
	if (pow(AB, 2) + pow(AC, 2) == pow(BC, 2) || pow(AB, 2) + pow(BC, 2) == pow(AC, 2) || pow(AC, 2) + pow(BC, 2) == pow(AB, 2))std::cout << "Триъгълникът е правоъгълен." << std::endl;
	if (pow(AB, 2) + pow(AC, 2)>pow(BC, 2) && pow(AB, 2) + pow(BC, 2)>pow(AC, 2) && pow(AC, 2) + pow(BC, 2)>pow(AB, 2))std::cout << "Триъгълникът е остроъгълен." << std::endl;
	if (pow(AB, 2) + pow(AC, 2)<pow(BC, 2) || pow(AB, 2) + pow(BC, 2)<pow(AC, 2) || pow(AC, 2) + pow(BC, 2)<pow(AB, 2))std::cout << "Триъгълникът е тъпоъгълен." << std::endl;

}

std::ostream & Triangle::inserter(std::ostream& out)const
{
	out << "Първата точка е:" << A << std::endl;
	out << "Втората точка е:" << B << std::endl;
	out << "Третата точка е:" << C << std::endl;
